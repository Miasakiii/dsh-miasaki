...
| Sidebar | [`dsh-miasaki-sidebar/`](dsh-miasaki-sidebar/) | DSH web 侧边栏插件 `@miasaki/dsh-sidebar`：**接入官方右侧 Sidebar**，提供审查 / 终端两个 tab 类型（辅助对话归 M2）。自研右栏壳已于 2026-09-10 退役、**2026-09-11 完成第二阶段清理**（壳代码删除 232 行，不再留死代码）；清理期间修复一处迁移遗留缺陷——审查视图此前写在已退役的壳 store 里，**视图下拉点了没有反应**，现改为自管持久化（`miasaki-sidebar:review-view`）。单测 40 项、静态回归 9/9。 |
