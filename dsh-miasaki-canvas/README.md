...
**M3 画布交互完成**（2026-09-05）：M2 合并内核之上补齐多选/框选、拖拽并置合并手势、吸收态标记、详情传递血缘视图；另完成 `<system-reminder>` 投影过滤与 0.1.2 存量回填（受限：仅覆盖会话变 live 后的补投影，纯内存历史回填等 DSH 上游接口，详见 CHANGELOG）。已适配 DSH 0.1.2-rc.1。剩余：M4 打磨（summary 注入形式、失败重试、回归测试、文档同步）。
