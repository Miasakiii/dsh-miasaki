...
const LEGACY_CARD_POSITIONS_KEY = 'dsh-canvas:card-positions'
const CARD_POSITIONS_KEY = 'dsh-canvas:card-positions:v3'
const COLLAPSED_CARDS_KEY = 'dsh-canvas:collapsed-cards:v1'
const QUICK_PHRASES_KEY = 'dsh-canvas:quick-phrases:v1'
...
const savedBranchAnchors = (() => {
  try {
    const value = JSON.parse(localStorage.getItem('dsh-canvas:branch-anchors') ?? '[]')
...
const savedCardPositions = (() => {
  try {
    ...
    localStorage.removeItem('dsh-canvas:card-positions:v2')
...
const state = {
  ...
  // 外部视图槽（2026-09-10）：其他插件写进宿主页面级注册表、再由 client 半
  // 经 `canvas:views` 下发的项。本页面只渲染按钮并回传点击，不解释 id 的语义。
  externalViews: [],
}
...
function post(type, payload = {}) {
  if (window.parent !== window) window.parent.postMessage({ source: 'dsh-canvas', type, ...payload }, window.location.origin)
}

function dshRpc(type, payload = {}) {
  if (window.parent === window) return Promise.reject(new Error('请从 DSH 页面打开 Canvas 后再操作会话'))
...
async function refreshSummaries({ renderAfter = true } = {}) {
  ...
  const body = await api('/canvas/api/workspaces')
...
async function openWorkspace(id, { renderAfter = true } = {}) {
  const load = ++state.workspaceLoad
  const body = await api(`/canvas/api/workspaces/${id}`)
...
async function sendMessage(thread, text) {
  ...
  render()
  try {
    await dshRpc('canvas:send-message', { sessionId: thread.dshSessionId, text })
...
async function submitDraft() {
  ...
  try {
    if (draft.kind === 'new') {
      const session = await dshRpc('canvas:create-session', { workspaceId: state.selectedDshWorkspaceId, cwd: state.currentDsh?.cwd })
      await dshRpc('canvas:send-message', { sessionId: session.id, text })
      state.draft = null
      render()
      window.setTimeout(() => {
    ...
    const session = await dshRpc('canvas:fork-session', { sessionId: parent.dshSessionId, atSeq: draft.atSeq })
    if (draft.anchorId !== undefined) rememberBranchAnchor(session.id, draft.anchorId)
    const result = await api(`/canvas/api/threads/${parent.id}/branch`, { method: 'POST', body: JSON.stringify({ title: text.slice(0, 42), dshSessionId: session.id, dshSessionTitle: session.title, position: branchPosition }) })
    ...
    render()
    await dshRpc('canvas:send-message', { sessionId: result.thread.dshSessionId, text })
    void loadThreadHistory(result.thread)
    await refreshProjection()
  } catch (error) {
...
function render() {
  ...
  // 外部视图槽：与「对话 / 会话布」并排的第三方入口。点击只广播 `canvas:view`，
  // 由注册方自己决定怎么切（本页面不认识任何具体视图）。
...
app.addEventListener('click', async event => {
  ...
  if (!(button instanceof HTMLElement)) {
    ...
    // without closing the map; the client confirms via canvas:current-session.
    if (thread.dshSessionId !== null) {
      ...
      post('canvas:activate-session', { sessionId: thread.dshSessionId })
    }
    return
  }
  ...
  try {
    ...
    if (button.dataset.action === 'close') post('canvas:close')
    if (button.dataset.action === 'external-view' && typeof button.dataset.viewId === 'string') post('canvas:view', { id: button.dataset.viewId })
    ...
    if (button.dataset.action === 'select-thread' && thread !== undefined) {
      ...
      // Bidirectional current-session sync: switch DSH's current session
      // without closing the map; the client confirms via canvas:current-session.
      if (thread.dshSessionId !== null) post('canvas:activate-session', { sessionId: thread.dshSessionId })
    }
...
app.addEventListener('change', event => {
  ...
  if (choice?.source === 'dsh') {
    ...
    void openDshWorkspace(choice.id).then(opened => {
      ...
      if (sessionId !== undefined) post('canvas:activate-session', { sessionId })
    }).catch(setError)
...
window.addEventListener('message', event => {
  if (event.origin !== window.location.origin || event.data?.source !== 'dsh-canvas') return
  const data = event.data
  if (data.type === 'canvas:map-opened') {
    ...
    state.mode = 'canvas'
    render()
    window.requestAnimationFrame(() => post('canvas:map-ready'))
  }
  ...
  if (data.type === 'canvas:views') {
    // 外部视图槽（2026-09-10）：宿主转来的第三方视图项。只渲染「对话 / 会话布」旁边那几个
    // 按钮，点击广播 `canvas:view` 由注册方处理 —— 本页面不认识任何具体视图。
    const items = Array.isArray(data.items)
      ? data.items
        .filter(item => item !== null && typeof item === 'object' && typeof item.id === 'string' && typeof item.label === 'string')
        .map(item => ({ id: item.id, label: item.label }))
      : []
    const unchanged = items.length === state.externalViews.length &&
      items.every((item, index) => item.id === state.externalViews[index].id && item.label === state.externalViews[index].label)
    if (!unchanged) {
      state.externalViews = items
  ...
  if (data.type === 'canvas:forked-session' || data.type === 'canvas:created-session' || data.type === 'canvas:message-sent') settleRpc(data.requestId, data.session ?? data)
  if (data.type === 'canvas:bridge-error') { settleRpc(data.requestId, undefined, new Error(data.message)); if (data.requestId === undefined) setError(data.message) }
})

post('canvas:request-current')
