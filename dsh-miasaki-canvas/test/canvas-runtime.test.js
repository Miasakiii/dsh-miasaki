...
test('reuses the live map iframe and retries initialization only after iframe load', async () => {
  ...
  assert.match(openFlow, /if \(mapOpening\) send\('canvas:map-opened'\)/)
  assert.ok(open.indexOf('overlay.hidden = false') < open.indexOf("send('canvas:map-opened')"))
...
test('keeps the canvas viewport across dialog/map toggles and recenters on real session switches', async () => {
  const source = await readFile(new URL('../app.js', import.meta.url), 'utf8')
  const mapOpened = source.slice(source.indexOf("if (data.type === 'canvas:map-opened')"), source.indexOf("if (data.type === 'canvas:workspaces')"))
  const currentSession = source.slice(source.indexOf("if (data.type === 'canvas:current-session')"), source.indexOf("if (data.type === 'canvas:live-reply'"))
...
test('selecting a session in the sidebar syncs the DSH current session', async () => {
  ...
  assert.match(selectThread, /canvas:activate-session/)
})
...
test('clicking a session card syncs the DSH current session', async () => {
  ...
  assert.match(cardClick, /post\('canvas:activate-session', \{ sessionId: thread\.dshSessionId \}\)/)
})
...
test('switching the workspace in the map syncs DSH to its first session', async () => {
  ...
  assert.match(select, /post\('canvas:activate-session'/)
})
...
test('mirrors DSH theme changes into the map', async () => {
  ...
  assert.match(clientSource, /canvas:theme/)
  assert.match(appSource, /data\.type === 'canvas:theme'/)
...
test('reveals hidden ancestor paths when a conversation becomes current', async () => {
  ...
  const current = source.slice(source.indexOf("data.type === 'canvas:current-session'"), source.indexOf("data.type === 'canvas:live-reply'"))
