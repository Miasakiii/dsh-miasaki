...
> 2026-09-24 更新：恢复入口已由「圆点」升级为**主题头像悬浮球**（`pet_native/dot.rs`，
> 56px 方窗/球面直径 38 + 主题色环 + 外发光 + 悬停放大；命中按球面 alpha 穿透）。
> M1.4 的四处同步点（显隐切换 / 散步自然结束 / 散步撞墙 / 拖动结束）语义不变，仍全部调
> `render_dot()`；见 `design/CHANGELOG.md` 2026-09-24 条目。
