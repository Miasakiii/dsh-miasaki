...
| 隐藏态恢复入口 = 主题头像悬浮球(2026-09-24) | 取代硬编码紫点(`0xB36AD9`/30px):球面 = **当前主题头像徽章**(`ui/icons/theme-*.png`,与设置「主题」选择器同源,编译期内嵌;`kurkuriel` 用 `theme-inverse.png`),外加主题色环 + 外发光 + 球面高光 + 底部落影,56px 方窗内球面直径 38。纯 CPU 预乘合成(`pet_native/dot.rs`,零 GDI 绘图/字体调用)。**球面缓冲即命中共据**:`dot_proc` 的 `IDT_HIT`(10ms)查 `dot_buf` alpha(阈值 16)驱动**同一命中**的穿透与悬停放大——球放大后不会留「隐形挡板」;主题经 `PetShared.theme` 落地,`spawn` 直接读 `prefs.json`(冷启动第一帧即正确球面),`compose` 比对 `dot_theme` 变化才重绘 |
