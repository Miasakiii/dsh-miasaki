...
//
// 用法：
//   node rebuild-baseline.mjs [--target <lib/index.js 路径>]
//
// 什么时候用：`node patch.mjs status` 报 `unknown`（DSH 升级覆盖了补丁），且
// `node patch.mjs verify` 因基线不匹配而失败时。它把**当前已安装的 lib/index.js**
// （升级后的官方原版）作为新基线，用 patch.mjs 的 applyPatch 纯函数试跑一次重建，
// 覆盖 baseline/index.original.js，并打印需要同步进 patch.mjs 的常量。
//
// 与 dsh-client-ui-* 系列补丁同一契约：**不存 patched 全文**，因此这里只写 original，
// 产物以 SHA 常量形式记录 —— verify 用「重建后的 SHA 是否等于常量」自证，与逐字节
// 比对等价（SHA 相等即逐字节相等）。
//
// 前提：锚点在新版中仍唯一命中。applyPatch 在锚点缺失或多重命中时抛错，**不会瞎改** ——
// 抛错就说明 EDITS 需要人工适配（见 README「升级后怎么办」）。本补丁的锚点是打包产物里
// 的 tab 缩进代码，DSH 若改动打包缩进或该段实现，锚点会失配并响亮报错。
//
...
import { applyPatch, TARGET_PACKAGE, TARGET_RELATIVE } from './patch.mjs'
...
if (target === undefined || target === null || !existsSync(target)) {
  console.error('[baseline] 未找到已安装的 lib/index.js（用 --target 指定，或设 MIASAKI_DSH_CORDIS_HOST_RUNNER）')
  process.exit(2)
}
...
try {
  ...
  console.error('[baseline] 提示：锚点漂移需人工适配 EDITS，见 README「升级后怎么办」')
  process.exit(1)
}
...
console.log(`[baseline] 已重建 baseline/index.original.js（${Buffer.byteLength(source, 'utf8')} B）`)
...
console.log('')
console.log('[baseline] 然后跑 node patch.mjs verify 自证，再 node patch.mjs apply 重打（之后需重启 DSH host）。')
