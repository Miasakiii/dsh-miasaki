...
> （**2026-09-23 后续**：基线已随 0.1.7-alpha.2 重打，见「基线沿革」与文末实操记录。）
...
> **实操记录（2026-09-23，实装 0.1.7-alpha.2 后重打）**：本机全局 DSH 已升 0.1.7-alpha.2，
> `status` 报 `unknown`。按上述动作执行：`rebuild-baseline.mjs`（原版 184,096 B，
> probe 自动选中 0.1.6+ 变体）→ 同步三个常量 → `verify` 三行 PASS → `apply` 成功
> （199,566 B，`9F2F1EE8…`）。**`EDITS` 13 条零改**——双代变体的前瞻适配在此兑现；
> legacy（0.1.5 内联 JSX）分支保留在规则中，回退到 0.1.5 时换回旧 baseline 即可再用。
> 实机待验：刷页面确认「思考强度」「获取可用模型/测试全部」两控件在位。
