...
//
// 与轨迹面板同一个缺口：chat 的 assistant 节点也只在**实时流式 chunk** 里折叠出
// `firstTokenTime`（lib/client.js 的 updateChunk → isTokenDelta 命中才记），窗口重建后为 null ——
// 于是消息气泡的「首 token 用时（TTFT）」（`assistantStepReading` 读 node.timing）
// 与 turnTime 面板一起失去该行，窗口口径的兜底统计 `deriveStats` 同样拿不到值。
//
// 该时间戳的实时来源是浏览器端 session controller 在流式过程中**合成**的 transient 事件
// `assistant/live-chunk`，**从不落盘**：
//
...
// 气泡里的 TTFT 消失。**数据并没有丢**：同一条 `assistant/message` 事件自带紧凑 stream
// （`data.stream`，text-chunks / reasoning-chunks / tool-call-chunks / chunk 记录，每条都带时间），
// 官方 host 侧统计投影 `dsh-session-stats` 正是用它恢复首 token 的
// （`assistantStreamFirstTokenTime`），所以「统计」对话框里的平均 TTFT 一直正常。
// 本包自己其实已经在读 `data.stream` —— 只是用来恢复**用量**（`streamUsage`），没顺带恢复时间。
//
// 本补丁只做一件事：给 chat 节点的 timing 补一条回退 —— 实时值没有时，从
...
// 姊妹补丁：../dsh-client-ui-trajectory（轨迹页三行计时走同一个 timing 字段）
 ...
 * 应用本补丁后的 SHA-256。
 *
 * 与 conversation 补丁同一取舍：**不存 patched 全文**（目标 361KB，再存一份不划算），
...
async function assertParses(source) {
	const file = join(tmpdir(), `dsh-chat-patch-check-${process.pid}.mjs`)
	try {
...
async function cmdStatus(args) {
	...
	if (target === null) {
		console.error('[patch] 未找到已安装的 client.js（用 --target 指定，或设 MIASAKI_DSH_CHAT_BUNDLE）')
		process.exitCode = 2
		return
	}
...
async function cmdApply(args) {
	...
	console.log('[patch] 提示   浏览器侧生效需刷新页面；消息气泡的首 token 用时（TTFT）随后可从日志恢复')
}
