...
| `10-contract.js` | —（2026-09-25 新增） | 桌面壳↔渲染层契约 **v1.1** `window.miasakiDesktop`（独立 IIFE；读能力 + 订阅 + **受控写能力**：写一律派发内部事件，由 `02-core`/`06-titlebar` 执行 —— 契约自己不碰 hash，写者数量不变；`design/desktop-contract.md`） |
