...
> **⚠ 2026-09-25 部分退役（读本文前先看这里）**：右栏 tab 形态已**退役**——官方右栏已内置终端
> （多标签 / Shell 选择 / 刷新后恢复），用户拍板「沿用官方策略，本项目不再做右侧边栏终端」
> （[CHANGELOG.md](CHANGELOG.md) 2026-09-25 第一条，v0.10.0-miasaki.0）。因此本文中所有
> 「**右栏 tab 容器**」相关表述（落点、§4.1 每容器独立活动标签、§6.3 容器模型、跨容器移位、
> 右栏 `×`、实例 `kind#N` 的右栏分栏/浮窗动因、可视原型里的右栏形态卡片）**均已失效**；
> 仍生效的是：会话集合 + 帧协议 v2（§5）、标签栏交互（§7）、快捷键（§7.3）、上限 8（§5.4）、
> 刷新恢复（§5.2）——它们全部服务于**底部面板单形态**。host 半（TerminalHub / WS / 路由）
> 与容器无关，零改动。
