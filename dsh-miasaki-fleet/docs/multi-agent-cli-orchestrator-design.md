...
> - v0.21 首次真实派单闭环（2026-09-11）：**能力闸门与 G0 节点交付契约首次在真实派单路径上落地**——积压 24 天的 `t-0003` 经派单前能力闸门（`claude` 覆盖 `coding`，score=105，候选 3 个）派给 claude，`claude -p` 24 回合 exit 0（$0.40396，in 86805 / out 11561 / cache-read 573696）；**本仓首个 `tasks/<id>/result.json`**（§4.8 契约首例）经 `validate-bus` 通过（**24 → 25 文件 0 错误**）；台账 2 条补丁 + `task.completed` 机器事件经 `bus-apply` 唯一入口**一次超步**提交（总线版本 **2 → 3**）；`verify-all fleet` **15/15 PASS**。**本批次未变更任何代码**——为既有协议（§4.8 / §11.2 / §7.0）的首次实践与交付物代写。**口径澄清**：headless 下 worker 无法落盘，但本轮 `permission_denials` 为 **0** ——其失败形态是 `Bash`/`Glob`/`Grep` 的 `EPERM: operation not permitted, uv_spawn …`（**进程 spawn 失败**），与 t-0006 记录的「Write **被权限栈拒绝**」是两类现象；§11.2 及相关档案 `preflight` 的措辞宜统一为「headless 下 worker 无法落盘」。worker 上报 7 项问题（含 `validate-bus.mjs` L231 对缺失 `result.json` 静默放行、派单器无法表达 `blocked` 终态、`notes.md` 口径矛盾、`context.md` 漂移）见 `docs/handover-2026-09-11.md` §8，**语义决策待 Operator 裁决**。
> - v0.22 第二批派单 + 首个契约边界发现（2026-09-11）：`t-0004` 同法闭环（claude 5 回合，**$0.22051**，74662 tokens），`result.json` 第二个实例，`validate-bus` → **26 文件 0 错误**，总线版本 **3 → 4**；**`task-ready --dispatchable` → 可派：无、终态 9 个** —— fleet 首次全部任务进入终态（24 天积压清零），当日总成本 **$0.624467**。**G0 指纹契约首次实际使用即撞出边界**：`agents/<id>/notes.md` 是被多任务共享、且被设计为**持续滚动**的追加文件，把它列入 `result.json` 的 `artifacts[]`（不可变产物指纹）会导致**每追加一次就让所有历史任务的指纹失效** —— 指纹机制被预期内的滚动噪声淹没，真篡改反而失去信号。处置：共享滚动文件**改列 `evidence[]`**（`note` 记载变更沿革），`artifacts[]` 只保留任务专属、内容稳定的产物；根治选项（notes 按任务分片 / 契约显式豁免）见 `docs/handover-2026-09-11.md` §9.2，**待 Operator 裁决**。另：§4.6 的 `notes.md` 口径矛盾（「总量 ≤10 行」vs brief「每次追加 ≤10 行」）本轮**真实撞上**（t-0003 十行 + t-0004 七行 = 17 行），暂按成文协议的总量口径**压缩合并至 10 行**并在文末标注待裁决，裁决权留 Operator。**本批次同样未变更任何代码**，且 worker 独立核对修正了简报的一处事实错误（源文档真实末次更新为 2026-08-17，非 08-16）。
> - v0.23 fleet-monitor 信任围栏（2026-09-26）：**全仓唯一「写接口零鉴权 + CORS 通配 `*`」的组合被收口**（仓库级评审报告 §五.P1.5 点名）。此前 `fleet-monitor/server.js` 的 `setCors` 无条件发 `Access-Control-Allow-Origin: *`，而 `POST /api/toggle/:agentId` **直接落盘** `agents/<id>/control.json` 却零鉴权 —— 只绑 `127.0.0.1` 限制的是「谁能连」，浏览器却可以把**任意网页**发起的请求送到环回地址，因此该写接口对用户访问过的任何页面都是敞开的。**修法**（与 appearance / ssh / sidebar 三条线同构；本线不 import 其它线代码，故为独立副本）：新增 `fleet-monitor/fence.cjs` 三道判定 —— ① Host 必须是 `localhost` / `127.0.0.1` 或 `FLEET_MONITOR_TRUSTED_HOSTS` 显式放行项；② `sec-fetch-site: cross-site` 一律拒；③ Origin 存在时其 hostname 必须与 Host 一致（或环回）。围栏排在**所有路由之前**（含静态面板与 preflight），被拒请求 **403 且一个 CORS 头都不带**；CORS 由通配改为**只回显已过围栏请求的 Origin**（附 `Vary: Origin`）。**可测性改造**：请求处理抽为具名 `handleRequest` 并导出，`listen` 由 `require.main === module` 包裹 ⇒ 单测可用假 req/res 驱动真实路由，**不起监听、不碰网络**（受限沙箱同样可跑；首版测试因假 req 的 `on()` 是空实现而挂在未结算的 `readBody` Promise 上，已修并写进注释）。新增 `tests/fleet-monitor.test.mjs` **11 例**：三道判定独立且 reason 可区分、`corsHeadersFor` 绝不出现 `*`、跨站写请求 403 且无 CORS 头、Host 非环回对 `/`·`/api/fleet`·`/api/report` 一律 403、preflight 同过围栏、**「过围栏才进业务分支」用不可写 agentId 反证**（ENOENT → 500 而非 403，且不落任何文件）。`verify-all fleet` **15 → 17 项**。
> - v0.23 附注 · 本线在新闸门下的存量（2026-09-26）：同批落地仓库级 `scripts/check-silent-guards.mjs`（「守卫必须显式失败」：R1 静默跳过 / R2 构建链静默吞错 / R3 静默回退读取 / R4 声明清单缺口；存量 **58 类冻结**在 `scripts/silent-guard-baseline.json`，**新增即失败**，`// guard-ok: <理由>` 可就地豁免）。**本线 23 处 R3 全部登记在案** —— `workers/graph/{verifier-pick,agent-pick,task-ready}.mjs`、`workers/bus/bus-apply.mjs`、`workers/lib/bus-apply-core.cjs` 与 `fleet-monitor/server.js` 大量以 `readJsonOrNull` / `safeReadJSON` 读 agent 档案，「档案不存在 = 该 agent 未初始化」是**正常语义**，故逐条冻结而非改写。**唯一例外是 `verifier-pick.mjs` 的 `loadVendors()`**：它读 `shared/agent-vendors.json`，而该文件**从不存在**，同时两处文档（本文件 v0.20 条与 `workers/lib/verifier.cjs` 注释）仍宣称「可由 `shared/agent-vendors.json` 覆盖」⇒ 与「删素材静默跳过」同型的**静默断链**（评审报告 §五.P1.9 给了二选一：补文件，或删 `loadVendors()` 与两处声明）。**本批未动**，留作「闸门驱动核销」的首个候选。
