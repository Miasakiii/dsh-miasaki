...
## 仓库结构（monorepo，2026-08-22 重组，2026-09-06 扩为四线）

本仓 `dsh-miasaki/` 单一 git 仓承载四条互不耦合的线 + 共享参考，各居其位：
...
四条线代码零耦合，仅共享 `dsh-miasaki-shared-docs/`。仓库内被 git 追踪的文件可写 `../dsh-miasaki-shared-docs/…` 形式的相对引用（同仓内，clone 后不断）。
