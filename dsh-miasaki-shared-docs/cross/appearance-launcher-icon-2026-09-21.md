...
- **内置预设图标也落在同一个目录**：`preset-<id>.png`（2026-09-21 M2.7 追加）。
  预设由 appearance 线程序化生成或来自其自带资源，落盘后与用户上传的图**完全同源** ——
  同一目录、同一文件名白名单、同一文件路由。**桌面壳对"预设"零感知，本契约不因它改变一个字。**
...
- 2026-09-21：A 侧 91 例单测、B 侧 25 例 `cargo test` 全绿（契约样本两侧一致）；
- 2026-09-21（M2.7 追加）：预设图标上线，**B 侧零改动**（无需重建 EXE 即可用上预设）——
  这正是"预设落盘到 avatars/"这一设计的验收点：壳侧代码与契约均未触碰，而用户多出可选图标
  （当前两款：默认 / 头像 —— 见 `lib/icon-presets.js` 的预设表）；
- 实机（上传 / 预设 → 任务栏 / 窗口 / 托盘图标跟随）**待用户重启 `dsh web` 后验收**。
