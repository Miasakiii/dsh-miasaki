...
| `dsh-miasaki-sidebar/README.md` + 壳死代码 | 大段「自研右栏壳」描述已自标失效，壳函数以死代码留存 | **已修**（2026-09-11 晚：壳代码删除 232 行、README 重写为官方右栏形态、退役 2 个测试文件并新增 1 个；**并修复视图下拉失效的迁移遗留缺陷**——详见 §5 注） |
...
| sidebar | 通过 | 通过 | 待复验 | 2026-09-11 完成第二阶段清理（壳代码删除 + 视图下拉缺陷修复），**修复项待实机复验** |
...
6. **文档同步（已完成）**：回归矩阵 §1 基线表、canvas README 版本号、sidebar README 与壳死代码
   均已处理（2026-09-11 晚完成第二阶段清理，见 §5 与 §3.2）。
