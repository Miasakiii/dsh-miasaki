# Datawhale《国产之光！GLM 5.3 + DeepSeek Harness 实测来了》归档

- 归档日期：2026-08-16
- 原发布：2026-08-15 22:00（北京时间）
- 来源：微信公众号「Datawhale」（user_name: gh_7e08dd400559）
- 作者：李秀奇（Datawhale 成员）
- 原文 URL：https://mp.weixin.qq.com/s/k1dAfy_uwxlazK70ecAaqA
- 获取方式：直连原文页面解析正文（微信对第三方抓取有验证码拦截）
- 关联文档：`docs/multi-agent-cli-orchestrator-design.md`（v0.7 已吸收其结论）
- 归档者：总指挥（Miasaki 会话）

---

Datawhale 干货 · 作者：李秀奇，Datawhale 成员

昨天智谱发布了最新大模型 GLM-5.3。据官方所说，GLM-5.3 在复杂软件工程、终端操作和更广泛的真实世界 Agent 任务上均取得非常显著的进步。
...
- 完整 PRD：九条生产级验收标准
- 保姆教程：DeepSeek Harness 保姆安装教程来了！
- GLM-5.3 获取地址：https://www.bigmodel.cn/glm-coding
...
[图片] GPT-5.6-Sol 版界面：暗色玻璃拟态风格
[图片] GLM-5.3 版界面：亮色扁平工具风格
...
[图片] GPT-5.6-Sol 版下拉菜单对比度缺陷
[图片] GLM-5.3 版同一位置：对比度正常
...
一个是**历史任务重跑**。GPT 版想调参数重跑同一张图，只能重新上传。GLM 版在服务端留存了原图和 trimap，调完参数点重跑就能生成新任务，前后对比很方便。调参的人都懂，同一张图反复跑是常态。

另一个是**批量评测**。一键跑 9 张示例图，弹窗出 SAD、MSE、Grad 的对照表，还能导出 CSV。改完一版算法想知道整体升了还是降了，点一下就行。

[图片] 批量评测功能：一键跑 9 张示例图出指标对照表
...
[图片] GLM-5.3+DeepSeek Harness：
[图片] GPT-5.6-Sol+DeepSeek Harness：

核心功能覆盖和算法基线效果两项打平。PRD 的要求两边都做了，算法也都是经典 closed-form 方法的合格实现。
...
智谱在技术博客里提到，GLM-5.3 的能力相比 GLM-5.2 整体提升了 50%，这部分提升全部来自后训练。放到这次测试里看，这个数字就有了具体的样子：登录限流、安全响应头、会话清理，以及需求之外主动补上的功能，都不是模型把代码写出来就够了，它还得知道一套真正要交付的应用应该长什么样。
